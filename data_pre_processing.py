...
def select_and_rename_target_activities(user_datasets, activity_mapping, verbose=0):
    """
    Select and rename target activities in the given user datasets.

    Parameters:
    - user_datasets (dict): Dictionary containing user datasets with subject as keys and data as values.
    - activity_mapping (dict): Mapping of original activity names to the desired standard names.
    - verbose (int, optional): If non-zero, print selected data information. Defaults to 0.

    Returns:
    - dict: Selected and renamed data with subject as keys and a list of tuples (data array, renamed labels) as values.
    - list: List of selected activity names.
    """

    # List of activities to select
    selected_activities = list(activity_mapping.keys())

    # Initialize an empty dictionary to store selected data
    selected_data = {}

    # Loop through each subject in the 'user_datasets' dictionary
    for subject, data_list in user_datasets.items():
        has_target_activity = False  # Flag to check if the subject has any target activities
        selected_data[subject] = []

        # Loop through each data tuple (array, labels) in the subject's data list
        for data_tuple in data_list:
            # Extract data arrays and labels
            data_array, labels = data_tuple

            # Mask to select rows corresponding to selected activities
            activity_mask = np.isin(labels, selected_activities)

            # Check if there is at least one selected activity
            if np.any(activity_mask):
                has_target_activity = True

                # Filter data based on the activity mask
                selected_data_array = data_array[activity_mask]
                selected_labels = labels[activity_mask]

                # Map old labels to new labels using activity_mapping
                selected_labels = np.array([activity_mapping[label] for label in selected_labels])

                # Append the selected data to the result dictionary
                selected_data[subject].append((selected_data_array, selected_labels))

        # Drop subjects without any target activities
        if not has_target_activity:
            del selected_data[subject]

    # Print the updated selected data
    if verbose:
        for subject, data_list in selected_data.items():
            print(f"Subject: {subject}")
            for i, (selected_data_array, selected_labels) in enumerate(data_list):
                print(f"Data Shape: {selected_data_array.shape}, Labels: {np.unique(selected_labels)}")

    return selected_data, np.unique(list(activity_mapping.values()))
...
def pre_process_dataset_composite(user_datasets, label_map, output_shape, train_users, test_users, window_size, shift, normalise_dataset=True, validation_split_proportion=0.2, verbose=0):
    ...
    Steps
        ...
        5: One-hot encode the labels (see np.eye for one-hot encoding)
    ...
    Parameters:
        ...
        output_shape
            ...
            (see np.eye for one-hot encoding)
        train_users
            list or set of users (corresponding to the user_id) to be used as training data
        test_users
            list or set of users (corresponding to the user_id) to be used as testing data
    ...
    train_y_one_hot = np.eye(output_shape)[train_y_mapped]
    test_y_one_hot = np.eye(output_shape)[test_y_mapped]
...
def pre_process_dataset_composite_in_user_format(user_datasets, label_map, output_shape, train_users, window_size, shift, normalise_dataset=True, verbose=0):
    ...
    Steps
        ...
        For each user:
            ...
            3: One-hot encode the labels (see np.eye for one-hot encoding)
    ...
    for user, user_dataset in user_datasets_windowed.items():
        ...
        # Step 3
        labels_one_hot = np.eye(output_shape)[labels_filtered]
