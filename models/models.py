...
class SWL_Discriminator(nn.Module):
    def __init__(self,configs):
        super().__init__()
        input_dim = configs.features_len * configs.final_out_channels
        self.L1 = nn.Linear(input_dim, 500, bias=False)
...
class HAR_CNN(nn.Module):
    ...
    def __init__(self,configs,use_standard_max_pooling=False):
        ...
        # Pooling layers
        self.use_standard_max_pooling = use_standard_max_pooling

    def forward(self, x):
        # Ensure batch dimension is present (avoid accidental squeezing)
        if x.dim() == 2:  # If input is (window_size, num_channels), add batch dim
            x = x.unsqueeze(0)
        # if x.dim() == 3:  # If input is (window_size, num_channels), add batch dim
        #     x = x.squeeze(0)

        # x = x.permute(1, 0)  # Change to (batch_size, num_channels, window_size)
        # x = x.permute(0, 2, 1)  # Ensure (batch_size, num_channels, window_size)
        ...
        if self.use_standard_max_pooling:
            # Standard max pooling over the entire window size (valid padding)
            x = F.max_pool1d(x, kernel_size=x.size(-1))
            # x = torch.flatten(x, start_dim=1)  # Reshape to remove the singleton dimension
        else:
            # Global max pooling to reduce the time dimension to 1
...
class CNN(nn.Module):
    def __init__(self, configs):
        ...
        #add maxpooling and dropout if specified in the data_model_configs.py file
...
#     def __init__(self, configs):
#         super(classifier, self).__init__()
#         self.dense = nn.Linear(configs.features_len * configs.final_out_channels, 1024)
